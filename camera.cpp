...
void Camera_MoveDirection(int forwardMove, int rightMove, float speed) {
	...
	if (forwardMove > 0) angle += rightMove > 0 ? M_PI / 4 : (rightMove < 0 ? -M_PI / 4 : 0);
	if (forwardMove < 0) angle += M_PI + (rightMove > 0 ? -M_PI / 4 : (rightMove < 0 ? M_PI / 4 : 0));
	if (forwardMove == 0) {
		angle += rightMove > 0 ? M_PI / 2 : -M_PI / 2;
